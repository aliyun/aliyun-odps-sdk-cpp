...
private:
    ...
    int write(LPGZIP buf, int count)
    {
        ...
        {
            int nBufferSize = m_CurrentBufferSize;
            while (Length + count > nBufferSize)
            {
                nBufferSize *= 2;
            }
            LPGZIP pTemp = pgzip;
            pgzip = static_cast<LPGZIP>(malloc(nBufferSize));
            if (!pgzip)
                throw std::bad_alloc();
            m_CurrentBufferSize = nBufferSize;
...
private:
    ...
    int gzread(char *buf, int len)
    {
        ...
        while (m_zstream.avail_out != 0)
        {
            if (m_transparent)
            {
                ...
                if (n > m_zstream.avail_out)
                    n = m_zstream.avail_out;
                if (n > 0)
                {
    ...
    int write(char *buf, int count)
    {
        ...
        if (Length + count > m_CurrentBufferSize)
        {
            ...
            char *pTemp = psz;
            psz = static_cast<char *>(malloc(nBufferSize));
            if (!psz)
                throw std::bad_alloc();
            m_CurrentBufferSize = nBufferSize;
            memset(psz, 0, m_CurrentBufferSize);
